...
def make_chains(text_string, n):
    ...
    for num in range(0, len(indiv_words)- n):

        for x in range(num,num+n):
            keys.append(indiv_words[x])
    ...
    #     # Check to see if the dictionary contains the key. If not, return [] 
    #     #and add the empty list as the value associated with the key.
    #     dict_chains[tuple_key] = dict_chains.get(tuple_key, [])

    #     # Append the value 'third_word' to the value
